...
if (!gotTheLock) {
  ...
  ipcMain.handle('start-server', async () => {
    ...
    try {
        ...
        ws.on('message', (message) => {
          ...
          switch (data.type) {
            ...
            case 'prevItem':
              ...
              break;
            case 'handwritingUpdate': broadcastToOthers(ws, { type: 'handwritingUpdate', payload: data.payload }); return;
            case 'presetMessage': broadcastToOthers(ws, { type: 'presetMessage', payload: data.payload }); return;
            case 'acknowledgement': if (directorWs && directorWs.readyState === directorWs.OPEN) { directorWs.send(JSON.stringify({ type: 'acknowledged' })); } return;
            case 'endProgram':
              programState = null;
              directorWs = null;
      ...
      function broadcastState() { if (programState) wss.clients.forEach(c => c.readyState === c.OPEN && c.send(JSON.stringify({ type: 'stateUpdate', payload: programState }))); }
      function broadcastToOthers(sender, msg) { wss.clients.forEach(c => c !== sender && c.readyState === c.OPEN && c.send(JSON.stringify(msg))); }
